...
public class CheCheckArrayIsSortedOrNot {
    public static void main(String[] args) {
        ...
        System.out.println("copied nums " + Arrays.toString(numsCopy));
        System.out.println("\n" + "Before sort numsCopy = " + Arrays.toString(numsCopy));
...
// ----------------------------------------------------------------------------------
        ...
        // Below was my exersises ...........................

        int[] numaralar = {11, 5, 7, 33, 61, 9, 33};
        int itemCount_numaralar = numaralar.length;
        int indexCount_numaralar = itemCount_numaralar-1;
        int [] yeniNumaralar = new int[itemCount_numaralar];

        for (int i = 0; i < itemCount_numaralar; i++) {
            yeniNumaralar[i] = numaralar[i];
        }
        System.out.println("yeni numaralar :" +Arrays.toString(yeniNumaralar));
        System.out.println("numaralar" + Arrays.toString(numaralar));

        Arrays.sort(yeniNumaralar);
        Arrays.sort(numaralar);

        numaralar = new int[]{1, 2, 3};

        if (Arrays.equals(yeniNumaralar, numaralar)){
            System.out.println("Arrays has matched succesfully");
        }else{
